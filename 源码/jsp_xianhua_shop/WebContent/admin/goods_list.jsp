...
<div class="container-fluid">
	...
	<div class="text-right"><a class="btn btn-warning" href="/admin/goods_add.jsp">添加鲜花</a></div>

	<br>

	<ul role="tablist" class="nav nav-tabs">
		<li <c:if test="${type==0 }">class="active"</c:if> role="presentation"><a href="/admin/goods_list">全部鲜花</a></li>
		...
		<li <c:if test="${type==3 }">class="active"</c:if> role="presentation"><a href="/admin/goods_list?type=3">新鲜花推荐</a></li>
	</ul>
			...
			<tr>
				...
				<td>
					<p>
						...
						<c:choose>
							<c:when test="${g.isNew }">
								<a class="btn btn-info" href="/admin/goods_recommend?id=${g.id }&method=remove&typeTarget=3&pageNumber=${p.pageNumber}&type=${type}">移出新鲜花</a>
							</c:when>
							<c:otherwise>
								<a class="btn btn-primary" href="/admin/goods_recommend?id=${g.id }&method=add&typeTarget=3&pageNumber=${p.pageNumber}&type=${type}">加入新鲜花</a>
							</c:otherwise>
						</c:choose>
